...
import { 
  ...
  Settings as SettingsIcon, Plus, Trash2, AlertCircle, CheckCircle2, Calculator 
} from 'lucide-react';
...
const Settings = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('http://127.0.0.1:8000/api/accounts/settings/', { headers: getAuthHeaders() })
  ...
  const handleSaveTemplate = async (e) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/grading/grading-templates/', {
        method: 'POST',
        headers: getAuthHeaders(),
      ...
      if (response.ok) {
        ...
        setItems([{ name: '', weight_percentage: '' }]);
        refreshTemplates();
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-w-3xl">
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between py-3 border-b border-gray-50">
              ...
              <select 
                name="active_school_year" value={settings.active_school_year} onChange={handleSettingsChange}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-50 bg-gray-50/50">
                <h2 className="text-xl font-serif font-bold text-[#1A1C29]">Create Grading Template</h2>
                <p className="text-sm text-gray-500 mt-1">Define flexible grade computations for your classes.</p>
              </div>
              ...
              <form onSubmit={handleSaveTemplate}>
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center space-x-2 px-6 py-2.5 rounded-xl text-sm font-bold bg-amber-400 hover:bg-amber-500 disabled:bg-amber-200 disabled:text-amber-700/50 text-[#1A1C29] transition-colors shadow-sm"
                  >
              ...
              <div className="p-6 space-y-4">
                {templates.length === 0 ? (
                  ...
                  templates.map(template => (
                    <div key={template.id} className="p-4 border border-gray-100 rounded-xl hover:shadow-md transition-shadow group cursor-default bg-white">
                      <h3 className="font-bold text-[#1A1C29] text-sm">{template.name}</h3>
                      <p className="text-[11px] font-bold text-gray-400 mt-1 uppercase tracking-wider">Transmutation Base: {template.transmutation_base}</p>

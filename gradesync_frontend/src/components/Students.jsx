...
const Students = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    program: 'BSCS', current_year_level: 1, section: 'Block A' 
  });
  ...
  const fetchGlobalStudents = () => {
    ...
    fetch('http://127.0.0.1:8000/api/grading/available-students/', {
      method: 'GET',
      headers: getAuthHeaders()
    })
      .then(res => res.json())
  ...
  const closeModal = () => {
    ...
    setFormData({ student_number: '', first_name: '', last_name: '', sex: 'F', email: '', program: 'BSCS', current_year_level: 1, section: 'Block A' });
                    ...
                    <div className="space-y-2">
                      {filteredGlobal.map(student => (
                        <div key={student.student_number} className="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-xl hover:shadow-sm transition-shadow">
                          ...
                          <button onClick={() => handleEnroll(student)} disabled={isSaving} className="px-4 py-1.5 bg-amber-100 hover:bg-amber-400 text-amber-900 font-bold text-sm rounded-lg transition-colors flex items-center gap-2">
